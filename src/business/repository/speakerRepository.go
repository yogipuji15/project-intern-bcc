...
func (h *speakerRepository) FindAll(filter entity.FilterParam, pagination entity.Pagination)([]entity.Speakers,*entity.Pagination,error){
	...
	err:= h.db.Order("rating desc").Joins("Category", h.db.Where(&entity.Categories{Category: filter.Category})).Where("name LIKE ? AND location LIKE ?", "%"+filter.Keyword+"%", "%"+filter.Location+"%").Offset(int(pg.Offset)).Limit(int(pg.Limit)).Find(&speakers).Error
	if err!=nil{
		return nil,nil,err
	}
